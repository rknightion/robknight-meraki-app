...
devices.go          GetOrganizationDevices, GetOrganizationDevicesStatusOverview, GetOrganizationDevicesAvailabilities
sensor.go           GetOrganizationSensorReadingsLatest, GetOrganizationSensorReadingsHistory, SensorReading* types
wireless.go         4 endpoint wrappers for MR (channel util, usage history, SSIDs, per-AP clients)
switches.go         3 endpoint wrappers for MS (port statuses bySwitch, port config, packet counters)
alerts.go           Assurance alerts (org list + overview-by-type)
*_test.go           Unit tests
export_test.go      Exposes internals for tests (TTLCache.clock override)
```
