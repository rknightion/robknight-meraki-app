...
test.describe('Meraki app navigation', () => {
  ...
  test('Sensors page renders variable selectors', async ({ gotoPage, page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: 'Sensors' })).toBeVisible();
    await expect(
      page.getByTestId('data-testid Dashboard template variables submenu Label Organization')
    ).toBeVisible();
  });

  test('Access Points page renders', async ({ gotoPage, page }) => {
    await gotoPage(`/${ROUTES.AccessPoints}`);
    await expect(page.getByRole('heading', { level: 1, name: 'Access Points' })).toBeVisible();
  });
  ...
  test('Alerts page renders severity selector', async ({ gotoPage, page }) => {
    ...
    await expect(
      page.getByTestId('data-testid Dashboard template variables submenu Label Severity')
    ).toBeVisible();
  });

  test('Appliances page renders', async ({ gotoPage, page }) => {
    await gotoPage(`/${ROUTES.Appliances}`);
    await expect(page.getByRole('heading', { level: 1, name: 'Appliances' })).toBeVisible();
  });
  ...
  test('Cellular Gateways page renders', async ({ gotoPage, page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: 'Cellular Gateways' })).toBeVisible();
  });
  ...
  test('Per-client drilldown route renders the MAC as page title', async ({
    ...
    await expect(page.getByRole('heading', { level: 1, name: mac })).toBeVisible();
  });

  test('Firmware page renders heading + tables', async ({ gotoPage, page }) => {
    await gotoPage(`/${ROUTES.Firmware}`);
    await expect(page.getByRole('heading', { level: 1, name: 'Firmware & Lifecycle' })).toBeVisible();
    ...
    await expect(page.getByText(/Pending upgrades|End-of-life devices/).first()).toBeVisible();
  });
  ...
  test('Traffic page renders device-type selector', async ({ gotoPage, page }) => {
    ...
    await expect(
      page.getByTestId('data-testid Dashboard template variables submenu Label Device type')
    ).toBeVisible();
  });
